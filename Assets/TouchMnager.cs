...
public class TouchMnager : MonoBehaviour
{
    // シングルトン
    private static TouchMnager _instance;

    private static TouchMnager Instance
    {
        get
        {
            if (_instance == null)
            {
                var obj = new GameObject(typeof(TouchMnager).Name);
                _instance = obj.AddComponent<TouchMnager>();
            }
            return _instance;
        }
    }
